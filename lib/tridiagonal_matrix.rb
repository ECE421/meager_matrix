...
class TridiagonalMatrix < SparseMatrix
  ...
  def self.rows(rows, column_count = rows[0].length)
    ...
    raise ArgumentError unless row_count == column_count && row_count > 2
  ...
  def determinant(n = @row_count)
    ...
    self[n - 1, n - 1] * determinant(n - 1) - self[n - 1, n - 2] * self[n - 2, n - 1] * determinant(n - 2)
  end
  ...
  def zero?
    @tridiagonal.each do |row|
      return false unless (row - [0]).empty? || row.empty?
    end
    true
  end
end
